...
<script lang="ts">
    ...
    // Give the global Quick Chat the current-page context (a quiz in this course).
    let contextToken: PageContext | null = null;
    $effect(() => {
        contextToken = setCourseContext("taking a quiz", state.courses.find((course) => course.id === params?.id));
    });
